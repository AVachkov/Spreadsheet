...
Formula::Formula(const std::string &formulaAsText) : formula_as_text(formulaAsText), result(), is_valid(false) {}

void Formula::solveFormula(const std::vector<std::string> &tokens_from_expression,
                           std::vector<char> &operators_from_expression, std::function<Number(Address)> getCellValue) {
    is_valid = false;
...
bool Formula::processOperations(std::vector<Number> &numbers_from_expression,
...
//     {
//         const Cell &c = (*cells)[address.row - 1][address.col - 1]; // address stored as actual spreadsheet indices
//         (starting from 1). switch (c.getType())
//         {
